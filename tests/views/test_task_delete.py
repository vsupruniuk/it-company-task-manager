...
class PublicTaskDeleteTests(TestCase):
    def setUp(self) -> None:
        self.url = reverse("manager:task-delete", kwargs={"pk": 1, "id": 1})

    def test_task_delete_login_required(self) -> None:
        res = self.client.get(self.url)

        self.assertEqual(res.status_code, 302)
        self.assertEqual("/accounts/login/?next=/projects/1/tasks/1/delete/", res.url)
...
class PrivateTaskDeleteTests(TestCase):
    def setUp(self) -> None:
        ...
        self.url = reverse(
            "manager:task-delete", kwargs={"pk": self.project.pk, "id": self.task.id}
        )
