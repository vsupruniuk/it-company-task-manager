...
class PublicTaskTypeDeleteTests(TestCase):
    def setUp(self) -> None:
        self.url = reverse("manager:task-type-delete", kwargs={"pk": 1})

    def test_task_type_delete_login_required(self) -> None:
        res = self.client.get(self.url)

        self.assertEqual(res.status_code, 302)
        self.assertEqual("/accounts/login/?next=/task-types/1/delete/", res.url)
...
class PrivateTaskTypeDeleteTests(TestCase):
    ...
    def test_should_delete_task_type(self) -> None:
        ...
        task_type = Project.objects.filter(name="Feature")

        self.assertEqual(list(task_type), [])
